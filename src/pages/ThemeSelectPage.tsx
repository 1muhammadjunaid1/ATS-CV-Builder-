...
function MiniPreview({ id }: { id: ThemeId }) { 
  ...
  const certifications = [
    { id: '1', name: 'Google UX Design Professional', issuer: 'Coursera', year: '2021' },
    { id: '2', name: 'Accessibility Advocate', issuer: 'W3C', year: '2022' }
  ]
  ...
  const c = { 
    ...
    experience: isEntryPreview ? experience.slice(1) : experience,
    education: [
    ...
    certifications: isEntryPreview ? certifications.slice(0, 1) : certifications,
...
export default function ThemeSelectPage() {
  ...
  const chooseWithKeyboard = (event: KeyboardEvent, id: ThemeId) => {
    if (event.key !== 'Enter' && event.key !== ' ') return
    event.preventDefault()
    choose(id)
  }
  return <main className="gallery">
    <nav className="brand"><Logo /></nav>
    <div className="gallery-intro">
      <p className="eyebrow">Choose your foundation</p>
      <h1>Pick a CV that feels like you.</h1>
      <p>Each layout is designed for sharp reading on screen, in print, and in applicant tracking systems.</p>
    </div>
    <section className="template-grid">
      {templates.map((template) => (
        <article
          className={`template-card template-${template.id}`}
          key={template.id}
          role="button"
          tabIndex={0}
          aria-label={`Use the ${template.name} CV template`}
          onClick={() => choose(template.id)}
          onKeyDown={(event) => chooseWithKeyboard(event, template.id)}
        >
          ...
          <div className="template-info">
            ...
            <div className="template-detail"><Check size={15} /> {template.detail}</div>
            <span className="template-action">Use this template <ArrowRight size={17} /></span>
          </div>
        </article>
      ))}
    </section>
    <p className="gallery-foot">Your CV stays in this browser until you choose to export it.</p>
  </main>
}
