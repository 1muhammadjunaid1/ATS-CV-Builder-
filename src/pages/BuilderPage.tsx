...
<div className="mobile-actions"><div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}><div><b>ATS Score <span>{ats.score}%</span></b><small>{sectionWarningsCount} warning{sectionWarningsCount !== 1 ? 's' : ''}</small></div><button className="theme-toggle" onClick={() => document.body.classList.toggle('dark')} style={{ border: 0, background: 'transparent', minHeight: 0, width: '32px', height: '32px', padding: 0 }}><Sun size={16} className="icon-light" /><Moon size={16} className="icon-dark" /></button></div><button className="btn-gemini" disabled={ai.usesLeft === 0} onClick={() => { setAiSuggestion(''); setAiOpen(true) }}><Sparkles size={14} /> Gemini AI</button><button onClick={exportToPDF}><Download size={14} /> PDF</button><button onClick={() => window.print()}><Printer size={14} /> Print</button><button onClick={() => setPreviewOpen(true)}><Eye size={14} /> Preview</button></div>
...
<Sun size={16} className="icon-light" /><Moon size={16} className="icon-dark" />
</button>
</header>
